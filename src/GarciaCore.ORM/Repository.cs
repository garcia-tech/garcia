...
namespace GarciaCore.ORM
{
    public class Repository
    {
        protected List<Type> cachingEnabledTypes = new List<Type>()
        {
            ...
            //typeof(Buyer),
            //typeof(BuyerProduct),
            //typeof(BuyerProductQuestion),
            ////typeof(CategoryQuestion),
            //typeof(City),
            //typeof(District),
            //typeof(Question),
            //typeof(PageContent),
            //typeof(Courier),
            //typeof(MemberCategory),
            //typeof(MenuItem),
            //typeof(MemberTypeMenuItem)
        };
        ...
        //public static string IdentitySqlStatement = ";select @@identity;";
        public static string IdentitySqlStatement = string.Format(" returning \"Id\";", TablePrefix);
        //public static string RowCountSqlStatement = "select @@rowcount;";
        public static string RowCountSqlStatement = string.Format(" returning \"Id\";", TablePrefix);
        public static string BeginTranSqlStatement = "begin transaction;";
        public static string CommitTranSqlStatement = "commit transaction";
        //public static string GetDateSqlStatement = "getdate()";
        public static string GetDateSqlStatement = "now()";
        private static string ColumnPrefix = "\"";
        private static string ColumnPpostfix = "\"";
        public static string TablePrefix = "\"";
        //public static string TablePrefix = "";
        public static string IdKeyword = string.Format("\"Id\"", TablePrefix);
            ...
            where T : Entity
        {
            ...
            sql += typeof(T).Name;
            sql += TablePrefix;
            sql += string.Format(" where {0}DeleteTime{0} is null", Repository.TablePrefix);
        ...
        protected virtual string GenerateDeleteScript<T>(T item) where T : Entity
        {
            ...
            sb += entityTypeName;
            sb += string.Format(" set {0}DeleteTime{0} = ", TablePrefix);
            sb += Repository.GetDateSqlStatement;
            sb += string.Format(" where {0}Id{0} = @Id and {0}DeleteTime{0} is null", TablePrefix);
            return sb.ToString();
        }
        ...
        private string GetTypeName(object item)
        {
            ...
            return TablePrefix + item.GetType().Name.PrepareName() + TablePrefix;
        }

        private string GenerateInsertScript(Entity item)
        {
            string script = item.GetInsertScript();
            ...
            if (string.IsNullOrEmpty(script))
            {
                ...
                var sb = new GarciaCoreStringBuilder(Repository.BeginTranSqlStatement);
                sb += "insert into ";
                string entityTypeName = this.GetTypeName(item);
                //sb += ColumnPrefix;
                sb += entityTypeName;
                ...
                {
                    propertyNames += ColumnPrefix;
                    propertyNames += property.Name.PrepareName();
                    propertyNames += ColumnPpostfix;
                    propertyNames += ",";
                    values += "@";
                ...
                sb += ")";
                sb += Repository.IdentitySqlStatement;
                sb += Repository.CommitTranSqlStatement;
                script = sb.ToString();
            }
        ...
        {
            string script = item.GetUpdateScript();
            ...
            if (string.IsNullOrEmpty(script))
            {
                ...
                sb += propertyNames.ToString().Trim().TrimEnd(',');
                sb += string.Format(" where {0}Id{0}=@Id", TablePrefix);
                script = sb.ToString();
            }
