...
namespace GarciaCore.ORM
{
    public class GarciaCoreRepository
    {
        ...
        {
            get { return _settings.CacheExpirationTimeInMinutes != 0; }
        }
        ...
        protected virtual async Task<List<T>> GetItemsFromDbAsync<T>(Dictionary<string, object> parameters,
            string order = null)
            where T : Entity
        {
            ...
            where T : Entity
        {
            ...
            foreach (Entity item in AllItems)
            {
                ...
                while (ienum.MoveNext())
                {
                    var p = item.GetType().GetProperty(ienum.Key.ToString(),
                        BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic);

                    if (p == null)
                    {
                        p = item.GetType().GetProperty(ienum.Key.ToString().Replace("Id", ""),
                            BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic);
                    }
            ...
            else if (item.IsDeleted)
            {
                ...
                using (var connection = _databaseConnection.CreateConnection())
                {
                    try
                    {
                        ...
                        var affectedRows = await connection.ExecuteAsync(script.ToString(),
        ...
        protected virtual PropertyInfo[] GetProperties(Entity Item)
        {
            ...
            return properties.Where(x => !Attribute.IsDefined(x, typeof(NotSavedAttribute)) &&
                                         !x.PropertyType.IsIList() && !x.PropertyType.IsArray && x.CanRead &&
                                         x.CanWrite &&
                                         (x.PropertyType.IsPrimitive || x.PropertyType.IsEnum ||
                                          x.PropertyType.Equals(typeof(String)) ||
                                          x.PropertyType.Equals(typeof(Decimal)) ||
                                          (Nullable.GetUnderlyingType(x.PropertyType) != null &&
                                           (Nullable.GetUnderlyingType(x.PropertyType).IsPrimitive ||
                                            ...
                                            Nullable.GetUnderlyingType(x.PropertyType).Equals(typeof(Decimal)))))
                                         && x.Name != "Id").ToArray();
        }
        ...
        private string GetTypeName(object item)
        {
            ...
            return _databaseConnection.TablePrefix + item.GetType().Name.PrepareName() +
                   _databaseConnection.TablePrefix;
        }
